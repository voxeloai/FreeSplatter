...
<#
.SYNOPSIS
  Create or attach a RunPod network volume + A40 pod for FreeSplatter.

.DESCRIPTION
  Native PowerShell port of scripts/deploy.sh. Idempotent on volume (reuses
  by name+DC); refuses to create a duplicate pod. No bash, no jq — uses
  PowerShell's built-in JSON support, with a JavaScriptSerializer fallback
  for the datacenter-list parse (PS 5.1's ConvertFrom-Json mangles the
  nested gpuAvailability arrays).
...
.EXAMPLE
  $env:HF_TOKEN = "<paste>"
  .\scripts\deploy.ps1 -DataCenter EU-RO-1 -PodName freesplatter-debug
#>

[CmdletBinding()]
param(
    [string] $PodName        = $(if ($env:POD_NAME)        { $env:POD_NAME }        else { 'freesplatter' }),
    [string] $VolumeName     = $(if ($env:VOLUME_NAME)     { $env:VOLUME_NAME }     else { 'freesplatter-workspace' }),
...
# ─────────────────────────────────────────────────────────────
# Create the pod (with retry — A40 stock fluctuates rapidly)
# ─────────────────────────────────────────────────────────────
...
Write-Host "  git clone https://github.com/voxeloai/FreeSplatter.git"
Write-Host "  cd FreeSplatter && git checkout voxelo/main"
Write-Host "  REPO_REMOTE=https://github.com/voxeloai/FreeSplatter.git bash scripts/bootstrap.sh"
Write-Host "  source /workspace/activate.sh"
Write-Host ""
Write-Host "For pod-shell sub-agent:"
Write-Host "  Spawn architect's pod-shell sub-agent with these connection details + cycle goal in .architect/task.md"
